...
object SampleDataProvider {
    ...
    fun getSamplePosts(): List<MarketingPost> {
    ...
    fun getSampleCampaigns(): List<WhatsAppCampaign> {
        val now = System.currentTimeMillis()
        val oneDayMillis = 86400000L
    ...
    fun getSampleVideoScripts(): List<VideoScript> {
        val now = System.currentTimeMillis()
        val oneDayMillis = 86400000L
    ...
    fun getInitialMetrics(): com.example.ui.viewmodel.DashboardMetrics {
        val posts = getSamplePosts()
        val campaigns = getSampleCampaigns()
        val drafts = posts.count { it.status == PostStatus.DRAFT.name }
        val pendingApproval = posts.count { it.status == PostStatus.PENDING_APPROVAL.name }
        val approved = posts.count { it.status == PostStatus.APPROVED.name }
        val scheduled = posts.count { it.status == PostStatus.SCHEDULED.name }
        val published = posts.count { it.status == PostStatus.PUBLISHED.name }
        val totalLeads = posts.sumOf { it.leadsGenerated } + campaigns.sumOf { it.responseCount }

        val now = System.currentTimeMillis()
        val sevenDaysAhead = now + (7 * 86400000L)
        val next7Days = posts.count {
            it.scheduledDateMillis != null && it.scheduledDateMillis in now..sevenDaysAhead
        } + campaigns.count {
            it.scheduledDateMillis != null && it.scheduledDateMillis in now..sevenDaysAhead
        }

        return com.example.ui.viewmodel.DashboardMetrics(
            draftsCount = drafts,
