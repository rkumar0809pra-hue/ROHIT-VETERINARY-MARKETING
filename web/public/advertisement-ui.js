...
let brief={title:'',topic:'',presenter:'',duration:'30',ratio:'9:16',language:'Hindi'};
...
export function advertisementSection(c){
 ...
 if(ad){
  ...
  <form id="ad-edit" data-id="${ad.id}" data-version="${ad.version}"><fieldset ${!editable||!owner?'disabled':''}>
  <label>Advertisement title<input name="title" value="${esc(ad.title)}" maxlength="160" required></label>
  <label>Presenter appearance / continuity notes<textarea name="presenter" maxlength="600">${esc(ad.presenter)}</textarea></label>
  <div class="metrics-grid"><label class="check"><input type="checkbox" name="narration" ${ad.narration?'checked':''}> Add AI narration (OpenAI usage)</label><label>Voice<select name="voice">${opts(['coral','onyx'],ad.voice,esc)}</select></label></div>
  ...
  <label>Scene direction<textarea name="prompt-${i}" maxlength="1800" required>${esc(s.prompt)}</textarea></label>
 ...
 }else content=`<form id="ad-create"><div class="metrics-grid"><label>Title<input name="title" value="${esc(brief.title)}" maxlength="160" required></label><label>Length<select name="duration">${opts(['30','60'],brief.duration,esc)}</select></label><label>Language<select name="language">${opts(['Hindi','English','Hinglish'],brief.language,esc)}</select></label><label>Format<select name="ratio">${opts(['9:16','16:9'],brief.ratio,esc)}</select></label></div><label>Topic and confirmed details<textarea name="topic" maxlength="2000" placeholder="Introduce our clinic, pet consultations and booking phone" required>${esc(brief.topic)}</textarea></label><label>Presenter description (optional)<textarea name="presenter" maxlength="600" placeholder="Same adult presenter, navy blazer, light blue shirt in every scene">${esc(brief.presenter)}</textarea></label><p>First prepare a script using one OpenAI request. Review scenes and costs before starting video generation. Upload clinic photos or app screenshots in Poster & media to use them here.</p><button class="primary" ${busy||!owner||!state.aiConfigured?'disabled':''}>${busy?'Preparing scenes…':'Prepare advertisement script'}</button></form>`;
...
export function advertisementChange(e,c){if(e.target.id==='ad-project'){selected=e.target.value;c.render();}}
...
export async function advertisementSubmit(form,c){
 ...
 try{
  if(form.id==='ad-create'){
   brief=Object.fromEntries(f);const ad=await c.api('advertisements','POST',{...brief,duration:Number(brief.duration)});selected=ad.id;
  }else if(form.id==='ad-edit'){
   const ad=c.state.advertisements.find(a=>a.id===id);
   await c.api('advertisements/'+id,'PUT',{version:Number(form.dataset.version),title:f.get('title'),presenter:f.get('presenter'),narration:f.has('narration'),voice:f.get('voice'),scenes:ad.scenes.map((s,i)=>({mode:f.get('mode-'+i),imageId:f.get('image-'+i),prompt:f.get('prompt-'+i),narration:f.get('narration-'+i)}))});
   ...
   if(ad.status==='draft'&&edit){const fields=new FormData(edit);if(fields.get('title')!==ad.title||fields.get('presenter')!==ad.presenter||fields.has('narration')!==ad.narration||fields.get('voice')!==ad.voice||ad.scenes.some((s,i)=>fields.get('mode-'+i)!==s.mode||fields.get('image-'+i)!==s.imageId||fields.get('prompt-'+i)!==s.prompt||fields.get('narration-'+i)!==s.narration))throw Error('Save your edited scenes before starting generation.');}
