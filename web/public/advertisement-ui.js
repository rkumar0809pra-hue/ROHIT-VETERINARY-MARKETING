...
let selected='',busy=false,step=1;
...
export function advertisementSection(c){
 ...
 }else content=`<form id="ad-create"><p class="step-progress">Step ${step} of 3 · ${["Your idea","Your photos","Presenter & voice"][step-1]}</p><section ${step!==1?'hidden':''}><h3>1. What is your video about?</h3><div class="metrics-grid"><label>Title<input name="title" value="${esc(brief.title)}" maxlength="160" required></label><label>Length<select name="duration">${opts(['30','60'],brief.duration,esc)}</select></label><label>Language<select name="language">${opts(['Hindi','English','Hinglish'],brief.language,esc)}</select></label><label>Format<select name="ratio">${opts(['9:16','16:9'],brief.ratio,esc)}</select></label></div><label>Topic and confirmed details<textarea name="topic" maxlength="2000" placeholder="Introduce our clinic, pet consultations and booking phone" required>${esc(brief.topic)}</textarea></label><button type="button" data-ad-prompt ${busy||!owner||!state.aiConfigured?'disabled':''}>Help me write my idea</button><p class="note">Optional: expands your idea using AI. Uses one text-generation request.</p></section><section ${step!==2?'hidden':''}><h3>2. Choose establishment photos (optional)</h3><p>Tick up to seven photos. AI can place matching photos into scenes as unchanged images with narration. Review its choices before generation.</p><div class="media-grid">${photos.map(m=>`<label class="card"><img loading="lazy" class="asset-preview" style="height:120px;object-fit:contain" src="/api/media/${m.id}/file" alt="${esc(m.title)}"><span><input type="checkbox" name="photoIds" value="${m.id}" ${(brief.photoIds||[]).includes(m.id)?'checked':''}> ${esc(m.title)}</span><small>${esc(m.category||'Unlabelled')} · ${esc(m.description||'')}</small></label>`).join('')||'<p>No clinic photos yet. Upload them in Poster & media, then return here.</p>'}</div></section><section ${step!==3?'hidden':''}><h3>3. Choose your presenter and voice</h3>${casting(brief,esc)}<details><summary>Optional: presenter appearance</summary><label>Presenter description (optional)<textarea name="presenter" maxlength="600" placeholder="Same adult presenter, navy blazer, light blue shirt in every scene">${esc(brief.presenter)}</textarea></label></details><p>First prepare a script using one OpenAI request. Review scenes and costs before starting video generation. Upload clinic photos or app screenshots in Poster & media to use them here.</p><button class="primary" ${busy||!owner||!state.aiConfigured?'disabled':''}>${busy?'Preparing scenes…':'Create my script'}</button></section><div class="actions">${step>1?'<button type="button" data-ad-step="back">← Back</button>':''}${step<3?`<button type="button" class="primary" data-ad-step="next" ${busy?'disabled':''}>${step===1?'Next: photos →':'Next: presenter →'}</button>`:''}</div></form>`;
 return `<section class="card"><div class="section-title"><div><h2>Create your clinic video</h2></div><button data-ad-new>Start a new video</button></div><p>Plan scenes, add your photos, then combine clips, narration, scene captions and clinic branding into one 720p video.</p><label>Saved projects<select id="ad-project"><option value="">Create a new advertisement</option>${rows.map(a=>`<option value="${a.id}" ${a.id===selected?'selected':''}>${esc(a.title)} — ${esc(a.status)}</option>`).join('')}</select></label>${content}</section>`;
}
...
export async function advertisementClick(el,c){
 if(el.dataset.adStep){
  const form=document.getElementById('ad-create');brief=readBrief(form);
  if(el.dataset.adStep==='next'&&step===1){for(const name of ['title','topic'])if(!form.elements[name].reportValidity())return true;}
  step=Math.max(1,Math.min(3,step+(el.dataset.adStep==='next'?1:-1)));c.render();document.querySelector('#ad-create')?.scrollIntoView({block:'start'});return true;
 }
 ...
 if(el.hasAttribute('data-ad-new')){selected='';step=1;c.render();return true;}
...
export async function advertisementSubmit(form,c){
 ...
 if(form.id==='ad-create'&&step<3)return advertisementClick({dataset:{adStep:'next'}},c);
